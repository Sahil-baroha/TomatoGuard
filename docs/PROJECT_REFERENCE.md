...
React + Vite (PWA) · FastAPI · PostgreSQL (Neon) · JWT (`aud`: `farmer` | `admin`) · Cloudinary (images) · pytesseract (server-side OCR only) · bcrypt==4.0.1 (pinned, do not change).
...
- `PATCH /profile` — farmer — in: any subset of profile/farm fields — out: updated object
- `POST /auth/change-password` — farmer — in: current_password, new_password
...
### Weather (Phase 5)
- `GET /weather/current` — farmer — 400 if farm has no lat/lng — writes to weather_records
- `GET /weather/forecast` — farmer — does NOT write to weather_records
...
### History (Phase 7)
- `GET /history?type=&from=&to=` — farmer — merged/sorted across all 4 data tables
