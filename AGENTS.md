...
docs/PROJECT_REFERENCE.md — condensed schema + endpoint contracts. Read this first, every session — it covers what you need for routine coding without the token cost of re-reading full prose specs.
docs/TomatoGuard_Execution_Playbook.md — tells you which phase you're on and what "done" means for it.
Only fall back to the full PDFs (docs/TomatoGuard_Database_Specification.pdf, _Backend_, _Frontend_) if PROJECT_REFERENCE.md doesn't resolve an ambiguity, or you need the reasoning behind a decision rather than just the fact. Don't read all three PDFs by default — that's expensive and usually unnecessary.
...
Scope your reads to what the current phase prompt lists

Every phase prompt from now on will include an explicit "Files to read" and "Files to touch" list. Read only those, plus PROJECT_REFERENCE.md if you haven't already this session. Do not open other files in the repo "just to check" unless something you're told to touch doesn't make sense without seeing a specific other file — and if that happens, say which file and why before opening it, rather than silently browsing the tree. This is a direct token-cost control, not a style preference.
...
Run whatever verification the playbook's checkpoint for that phase asks for.
Report the actual output (query results, curl responses, screenshots) — not just "it works."
Stop and wait for explicit confirmation before starting the next phase, even if auto-continue is enabled. Phase boundaries in the playbook are deliberate checkpoints, not suggestions — do not chain multiple phases into one uninterrupted run.
